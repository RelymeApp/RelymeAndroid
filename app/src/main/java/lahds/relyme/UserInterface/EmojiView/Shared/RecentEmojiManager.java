...
package lahds.relyme.UserInterface.EmojiView.Shared;
...
public final class RecentEmojiManager implements RecentEmoji {
    ...
    public boolean isEmpty() {
        ...
        if (!EmojiManager.isShowingEmptyRecentEnabled()) {
            return true;
        }
        return false;
    }
    ...
    public void sortEmoji() {
        ...
        for (HashMap.Entry<String, Integer> entry : emojiUseHistory.entrySet()) {
            recentEmoji.add(EmojiManager.getInstance().findEmoji(entry.getKey()));
        }
