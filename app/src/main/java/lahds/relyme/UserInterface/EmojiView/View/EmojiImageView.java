...
public final class EmojiImageView extends AppCompatImageView {
    ...
    private boolean asyncLoad = EmojiManager.isAsyncLoadEnabled();

    boolean showVariants = EmojiManager.getEmojiViewTheme().isVariantDividerEnabled();
    ...
    private void init() {
        variantIndicatorPaint.setColor(EmojiManager.getEmojiViewTheme().getVariantDividerColor());
        ...
        if (EmojiManager.isRippleEnabled()) Utils.setClickEffect(this, false);
    }
    ...
    public void setEmoji(@NonNull final Emoji emoji) {
        if (!emoji.equals(currentEmoji)) {
            ...
            if (EmojiManager.getEmojiLoader() != null) {
                EmojiManager.getEmojiLoader().loadEmoji(this, emoji);
            } else {
                if (asyncLoad) {
                    imageLoadingTask = new ImageLoadingTask(this);
                    imageLoadingTask.execute(emoji);
