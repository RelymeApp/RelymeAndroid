...
import lahds.relyme.UserInterface.EmojiView.Listener.PopupListener;
import lahds.relyme.UserInterface.EmojiView.Search.EmojiSearchView;
import lahds.relyme.UserInterface.EmojiView.Utils.Utils;


public class EmojiPopupLayout extends FrameLayout implements PopupInterface {


    EmojiPopupView popupView;
    ...
    public EmojiPopupLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public EmojiPopupLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        Utils.forceLTR(this);
        initKeyboardHeightProvider();
    }

    public void initPopupView(EmojiBase content) {
        if (keyboard == null) {
            keyboard = new View(getContext());
            this.addView(keyboard,new FrameLayout.LayoutParams(-1,0));
        }
        content.setPopupInterface(this);
        popupView = new EmojiPopupView(this, content);
    ...
    protected static class KeyboardHeightProvider extends PopupWindow {
        ...
        private EmojiPopupLayout layout;

        /**
         * Construct a new KeyboardHeightProvider
         */
        public KeyboardHeightProvider(EmojiPopupLayout popupLayout) {
