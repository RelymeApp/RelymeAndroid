...
public class EmojiRadioButton extends AppCompatRadioButton {
    ...
    public EmojiRadioButton(final Context context, final AttributeSet attrs) {
        ...
        if (attrs == null) {
            ...
            final TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.EmojiRadioButton);

            try {
                emojiSize = a.getDimension(R.styleable.EmojiRadioButton_emojiSize, defaultEmojiSize);
            } finally {
                a.recycle();
            }
        }
